    ...
    try {
      ...
      if (error) {
        ...
        try {
          ...
          const bookingItems: BookingItem[] = cartItems.map(item => ({
            productId: item.productId,
            quantity: item.quantity
          }));

          const bookingResponse = await axios.post<BookingResponse>("/api/users/bookings", { 
            items: bookingItems
          });
  ...
  useEffect(() => {
    const fetchPaymentData = async () => {
      try {
        ...
        const cartRes = await axios.get<CartResponse>("/api/users/cart");
        ...
        const intentRes = await axios.post<PaymentIntentResponse>("/api/users/check-out-session", { 
